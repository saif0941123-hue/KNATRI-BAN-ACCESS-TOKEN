...
╚═╝  ╚═╝╚═╝  ╚═══╝╚═╝  ╚═╝   ╚═╝   ╚═╝  ╚═╝╚═╝
    ...
    print(ascii_art) 
    draw_line()
    print(f"{W}{B}             FF ㅤＫＮＡＴＲＩ BAN {S}")
    draw_line()

def show_credits():
    draw_line()
    print(f"\n {C}[★] {W}Developer : {C}@knatri77{S}")
    print(f" {C}[★] {W}Join      : {G}https://t.me/knatri_77{S}")
    print(f" {C}[★] {W}Join      : {G}https://t.me/knatri_777{S}\n")
...
def get_base_url(lock_region):
    ...
    Same logic as the login history extractor script.
    """
...
def main():
    ...
    while True:
        ...
        try:
            ...
            def do_inject():
                return trigger_injection(jwt_token, version, base_url)
